/-
Kairos.Stats.EDetector — E-detector framework for sequential change detection.
...
Tier 2 / sequential stats scaffold (2026-04-25, ATH-606).
All five statements are `sorry`-ed with flagged closure plans.
**Excluded from `Kairos.Stats.AxiomAudit` until closures land.**
-/
import Mathlib
...
Hence M is an e-process; this bridges `Kairos.Stats.SubGaussianMG` to the
...
`SubGaussianMG.exp_supermartingale` construction in `Kairos.Stats.SubGaussianMG`.
