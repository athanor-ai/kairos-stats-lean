...
Customer use (per ATH-XXX, [customer] LLM-orchestrated RTL area
...
Architecture: the LLM (qa-lane in ATH-XXX) generates candidate
abstraction functions. This module's typing kernel-checks them.
The SEC harness (kairos.sec, ATH-XXX) consumes them as EBMC
`assume property` directives.

Reference: ATH-XXX epic (parent), ATH-XXX (cross-arm child for
...
on the canonical [customer] optimization (FIFO implemented as
