...
When the `pythia.machineFormat` Lean option is set, it ALSO emits a
tagged log line `[pythia.result] {"rung": ...}` (success) or
`[pythia.failure] {"reason": ...}` (failure) carrying structured
...
`set_option pythia.machineFormat true`, so interactive sessions
...
set_option pythia.machineFormat true in
example : (1 : ℝ) ≤ 1 := by pythia?
